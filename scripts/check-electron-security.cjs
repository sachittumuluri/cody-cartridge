...
function main() {
  ...
  assert((main.match(/ipcMain\.handle\("music:/g) ?? []).length === 8, "Electron shell exposes expected music IPC handler count");
  assert((main.match(/ipcMain\.handle\("music:[^"]+", withTrustedRenderer/g) ?? []).length === 8, "All music IPC handlers are trusted-renderer guarded");
